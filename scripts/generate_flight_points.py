...
def emit(final, out_path):
    ...
    for uid, entry in sorted(final.items(), key=lambda kv: (kv[1]["continent"], kv[0])):
        ...
        tail = ""
        if alt:
            # The alternate repeats continentID so a consumer that picks one of
            # the two never has to know which half it is holding -- and it is
            # the ALTERNATE's own world map, not the primary's. Writing the
            # primary's would invert SameFlightNetwork for any zone whose two
            # nodes sit on different world maps. No zone does today, so the
            # emitted file is identical either way; the difference is only
            # visible the day one does.
            tail = (f', alt = {{ {render(alt)}, '
                    f'continentID = {alt["continent"]}, '
