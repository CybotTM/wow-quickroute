...
- Clicking a teleport cloak you are not wearing no longer reports that the item was not found. Equipping and using cannot happen in one click — the item reaches the slot only after the server answers — so the first click equips and says so, and the second teleports.
