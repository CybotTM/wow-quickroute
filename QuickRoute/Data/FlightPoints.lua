...
--
-- Three filters, in order, because TaxiNodes holds far more than flight
-- masters. A node is kept only when:
--
--   1. It has at least two neighbours in TaxiPath. A flight master connects to
--      many nodes; a scripted quest flight connects to exactly one, and the
--      boat and zeppelin nodes ("Transport, Menethil Harbor") to none. This
--      alone removes 658 of 1464, including every "Filming", "[DISABLED in
--      4.x]" and "Quest - ..." row.
--   2. Some zone-type UiMap box contains its position; the smallest wins.
--   3. Its own name corroborates that zone. A node named "Place, Zone" states
--      where it is, so the geometric answer has to agree with it -- either the
--      zone part names the zone, or the place part does, which is the city
--      inside a zone ("Ironforge, Dun Morogh" is in the Ironforge zone). Zone
--      boxes overlap at borders, and without this a flight master lands in the
--      neighbouring zone: "New Kargath, Badlands" was being filed under
--      Searing Gorge, 1103 yards away. This removes a further 231.
--
-- The rule is deliberately conservative: a node whose name cannot corroborate
-- the geometry is dropped, not guessed at. That costs real entries -- Boralus
-- and Dazar'alor go, because their flight masters are named "Proudmoore Keep"
-- and "Port of Zandalar" -- and it is the right trade, because a wrong entry
-- makes the addon assert a flight master that is not there and hand out a
-- route nobody can fly.
--
-- One entry per zone: the most connected node wins, lowest node ID breaks ties
-- so regeneration is stable. The graph is zone-level, and from any flight
-- master the game auto-routes multi-hop to every point you have discovered on
-- that world map, so the per-path topology adds nothing at this granularity.
--
...
--
-- Regenerate: scripts/generate_flight_points.py
local ADDON_NAME, QR = ...
...
QR.FlightPoints = {
    ...
    [77] = { x = 0.4770, y = 0.6505, worldX = 5123.5, worldY = -321.2, continentID = 1, node = "Bloodvenom Post, Felwood [DISABLED in 4.x]" },
    ...
    [461] = { x = 0.0679, y = 0.0042, worldX = -838.8, worldY = -4986.0, continentID = 1, node = "Durotar - ET - CC Prologue Troll Taxi Bat Start" },
    -- world map 530
    ...
    [127] = { x = 0.6207, y = 0.6347, worldX = 5813.9, worldY = 449.1, continentID = 571, node = "Dalaran" },
    -- world map 870
    ...
    [646] = { x = 0.6662, y = 0.8388, worldX = -860.8, worldY = 4296.0, continentID = 1220, node = "Dalaran" },
    ...
    -- world map 2374
    [1970] = { x = 0.6477, y = 0.5049, worldX = -3231.1, worldY = -1.9, continentID = 2374, node = "[HIDDEN] - 9.2 Resonant Peaks - Teleport Network - Hidden Hub (Connects all Nodes to each other without unique paths) (MAH/JDA)" },
    -- world map 2444
    [2022] = { x = 0.3197, y = 0.4219, worldX = 1753.2, worldY = 42.1, continentID = 2444, node = "Ruby Life Pools, The Waking Shores" },
    [2023] = { x = 0.9048, y = 0.1980, worldX = 942.0, worldY = 746.0, continentID = 2444, node = "[Hidden] 10.0 Travel Network - Destination Input" },
    ...
    -- world map 2657
    [2298] = { x = 0.7635, y = 0.4927, worldX = -2843.1, worldY = -396.6, continentID = 2657, node = "Grand Rampart" },
    -- world map 2694
    ...
    -- world map 2738
    [2371] = { x = 0.6695, y = 0.2412, worldX = 1391.8, worldY = -1040.2, continentID = 2738, node = "The Oasis" },
