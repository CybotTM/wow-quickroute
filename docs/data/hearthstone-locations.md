...
Checked on 2026-09-12 against retail build **12.1.0.69587**. The data contains **58 approximate inn positions on 43 maps** and **61 explicit ambiguity/phase blockers** (119 records). A runtime locale can have fewer usable names: translated aliases deliberately block otherwise unique English names.

This is a conservative initial catalog, not a complete list of WoW inns. Every accepted position comes from an explicit hearth-binding step with a numeric UI map ID in the authors' current retail guides. Names in guide instructions and NPC names are not interchangeable. Runtime matching uses the client's localized `C_Map.GetAreaInfo(areaID)` and the exact `GetBindLocation()` string; no English alias or NPC-name matching is required.
...
The offline audit found all 119 catalog area IDs present and nonempty in every locale. It resolves 58 binding names in enUS, deDE, frFR, esES, esMX and zhTW, and 57 in ptBR, ruRU, koKR, zhCN and itIT. The latter locales have one additional collision affecting a coordinate-bearing entry. The test fixture `tests/fixtures/hearthstone_area_names.lua` contains all 119 EN/DE labels extracted directly from these snapshots; `tests/test_hearthstone_catalog.lua` exercises the shipped catalog through the real resolver.
...
The checked-in `scripts/audit_hearthstone_catalog.py` validates these facts without executing source Lua. Place the eleven AreaTable CSV exports above in a source directory as `AreaTable-<locale>.csv`, the same-build UiMap export as `UiMap.csv`, and the [pinned guide archive](https://codeload.github.com/Ludovicus-Maior/WoW-Pro-Guides/tar.gz/f7c84c7ef4bcc3af20ff33cb5a497dd639b45851) as `WoW-Pro-Guides-f7c84c7.tar.gz`. From the repository root, run:

```sh
python3 scripts/audit_hearthstone_catalog.py --root . --sources /path/to/source-directory
```

The audit checks every coordinate against an explicit source binding step, verifies every UI map and area ID, checks translated collisions against the complete AreaTable, and prints source checksums. `--fixture tests/fixtures/hearthstone_area_names.lua` regenerates the EN/DE test fixture from the same client data. The ordinary Lua suite uses that fixture without requiring network access or the complete snapshots.
...
| Fairbreeze Village | 3462, 15995 | 94: 43.70, 71.20 | AreaTable name has multiple area IDs. | [INTRO_Blood_Elf.lua:122](https://github.com/Ludovicus-Maior/WoW-Pro-Guides/blob/f7c84c7ef4bcc3af20ff33cb5a497dd639b45851/WoWPro_Leveling/Retail/Horde/INTRO_Blood_Elf.lua#L122) |
...
For the Midnight village, the indexed first-party [Dragon Army travel notes](https://dragonarmyguild.net/viewthread.php?p=1&threadid=4847) report an innkeeper at **46.3, 46.0**; direct retrieval had a certificate hostname error during this audit. This is a research lead, **not executable catalog data**, and does not establish the individual landing point. The catalog explicitly blocks both area IDs. A localized binding name alone cannot distinguish them; the player's current map, level or faction is not proof of the saved binding's world instance. Rebinding near the intended inn, an actual arrival observation, or a future verified destination API can disambiguate it.
...
- All catalog results remain fallbacks. Unknown/ambiguous labels continue to require stronger evidence; observed character data is never overwritten by this catalog.
